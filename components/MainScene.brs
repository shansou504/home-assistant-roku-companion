sub init()
    ...
    light = m.domainContent.createChild("ContentNode")
    light.id = "light"
    light.setField("title", "Lights")
...
sub UpdateSecondPanel()
    ...
    if m.domain <> invalid
        if m.domain.id = "binary_sensor"
            ...
            m.pass = true
        else if m.domain.id = "switch" or m.domain.id = "light"
            m.domainListPanel.nextPanel = m.switchListPanel
            m.postRequestToggleSwitch.url = m.baseurl + "services/" + m.domain.id + "/toggle"
            m.pass = true
        end if
    end if
    if m.pass
...
sub UpdateSecondPanelContent()
    if m.getStates.response <> invalid and m.getStates.response <> "" and m.domain <> invalid and (m.domain.id = "binary_sensor" or m.domain.id = "light" or m.domain.id = "switch")
        ...
        if json <> invalid
            ...
            for each item in itemArray
                ...
                node.addField("domain", "string", false)
                node.setField("domain", m.domain.id)
            ...
            else if m.domain.id = "light" or m.domain.id = "switch"
